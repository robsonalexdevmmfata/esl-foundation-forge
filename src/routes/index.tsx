      ...
      <main>
        ...
        <section id="inicio" className="relative isolate">
          ...
          <div className="relative mx-auto max-w-4xl px-5 py-28 text-center md:py-40">
            <span className="inline-block rounded-full border border-mostarda px-4 py-1 font-display text-xs font-bold uppercase tracking-[0.2em] text-mostarda">
              Engenharia & Construção
            </span>
            <h1 className="mt-6 text-4xl font-extrabold leading-tight text-background md:text-6xl">
              ESL FACILITY: Construindo o Futuro com Excelência e
              Sustentabilidade.
            </h1>
            <p className="mx-auto mt-5 max-w-2xl text-base text-background/85 md:text-lg">
            ...
            <div className="mt-9 flex flex-col justify-center gap-3 sm:flex-row">
              <a
                href="#sobre"
                className="rounded-md bg-floresta px-7 py-3.5 font-display text-sm font-bold text-background ring-1 ring-mostarda/50 transition-colors hover:bg-floresta-dark"
              >
                Saiba Mais
              </a>
              <a
                ...
                className="rounded-md bg-terra px-7 py-3.5 font-display text-sm font-bold text-background transition-colors hover:bg-terra-dark"
              >
        ...
        <section id="sobre" className="bg-background py-20 md:py-28">
          <div className="mx-auto grid max-w-6xl items-center gap-12 px-5 md:grid-cols-2">
            <div>
              <p className="font-display text-xs font-bold uppercase tracking-[0.25em] text-terra">
                Sobre Nós
              </p>
              <h2 className="mt-3 text-3xl font-extrabold text-floresta md:text-4xl">
                Nossa Missão
              </h2>
              ...
              <dl className="mt-8 grid grid-cols-3 gap-4 border-t border-mostarda pt-6">
                {[
                  ["15+", "anos de mercado"],
                  ["320", "obras entregues"],
                  ...
                  <div key={l}>
                    <dt className="font-display text-3xl font-extrabold text-terra">
                      {n}
                    </dt>
            ...
            <div className="relative">
              <div className="absolute -left-4 -top-4 h-24 w-24 border-l-4 border-t-4 border-mostarda" />
              <div className="absolute -bottom-4 -right-4 h-24 w-24 border-b-4 border-r-4 border-mostarda" />
              <img
                src={equipe}
          ...
          <div className="mx-auto max-w-6xl px-5">
            ...
            <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {servicos.map((s, i) => (
                <article
                  key={s.titulo}
                  className="group rounded-md border border-mostarda bg-card p-7 transition-shadow hover:shadow-lg"
                >
                  <div className="flex h-11 w-11 items-center justify-center rounded-md bg-floresta/10 font-display text-lg font-extrabold text-terra transition-colors group-hover:bg-terra group-hover:text-background">
                    {String(i + 1).padStart(2, "0")}
                  </div>
                  <h3 className="mt-5 font-display text-lg font-bold text-floresta">
                    {s.titulo}
                  </h3>
                  <p className="mt-2 text-sm text-foreground/80">{s.texto}</p>
                </article>
              ))}
            </div>
          </div>
        ...
        {/* PROJETOS */}
        <section id="projetos" className="bg-floresta py-20 md:py-28">
          <div className="mx-auto max-w-6xl px-5">
            <p className="font-display text-xs font-bold uppercase tracking-[0.25em] text-mostarda">
              Portfólio
            </p>
            <h2 className="mt-3 text-3xl font-extrabold text-background md:text-4xl">
              ...
              {projetos.map((p) => (
                ...
                >
                  ...
                  <div className="absolute inset-x-0 bottom-0 p-6">
                    ...
                    <p className="mt-1 max-h-0 overflow-hidden text-sm text-background/90 opacity-0 transition-all duration-300 group-hover:max-h-24 group-hover:opacity-100">
                      {p.resumo}
                    </p>
                  </div>
                </article>
        ...
        {/* CONTATO */}
        <section id="contato" className="bg-mostarda-soft py-20 md:py-28">
          <div className="mx-auto grid max-w-6xl gap-12 px-5 md:grid-cols-2">
            <div>
              <h2 className="text-3xl font-extrabold text-floresta md:text-4xl">
                Pronto para dar vida ao seu projeto?
              </h2>
              <p className="mt-4 text-carvao/85">
                Conte o que você quer construir. Em até 24 horas úteis um
                engenheiro da ESL FACILITY retorna com os próximos passos e uma
                estimativa realista.
              </p>
              <ul className="mt-8 space-y-3 font-display text-sm font-semibold text-floresta">
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="mt-8 inline-flex rounded-md border-2 border-floresta px-6 py-3 font-display text-sm font-bold text-floresta transition-colors hover:bg-floresta hover:text-background"
              >
            ...
            <form
              className="rounded-md bg-card p-7 shadow-sm"
              onSubmit={(e) => {
                e.preventDefault();
            ...
            >
              <label className="block font-display text-xs font-bold uppercase tracking-widest text-floresta">
                Nome
                <input
                  name="nome"
                  required
                  className="mt-2 w-full rounded-md border border-input bg-background px-3 py-2.5 font-sans text-sm font-normal text-foreground outline-none focus:border-terra"
                />
              </label>
              <label className="mt-5 block font-display text-xs font-bold uppercase tracking-widest text-floresta">
                E-mail
                <input
                  name="email"
                  type="email"
              ...
              <button
                type="submit"
                className="mt-6 w-full rounded-md bg-terra px-6 py-3 font-display text-sm font-bold text-background transition-colors hover:bg-terra-dark"
              >
                Enviar mensagem
              </button>
            </form>
